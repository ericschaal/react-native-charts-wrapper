...
public abstract class DataExtract<D extends ChartData, U extends Entry> {
    ...
    public D extract(Chart chart, ReadableMap propMap) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            if (BridgeUtils.validate(dataSetReadableMap, ReadableType.String, "xAccessor")) {
                xAccessor = dataSetReadableMap.getString("xAccessor");
            }

            if (BridgeUtils.validate(dataSetReadableMap, ReadableType.String, "yAccessor")) {
                yAccessor = dataSetReadableMap.getString("yAccessor");
            }



            ArrayList<U> entries = null;

            if (!xAccessor.isEmpty() && !yAccessor.isEmpty()) {
    ...
    ArrayList<U> createEntries(ReadableArray yValues, String xAccessor, String yAccessor) {
        ArrayList<U> entries = new ArrayList<>(yValues.size());
        for (int j = 0; j < yValues.size(); j++) {
            if (!yValues.isNull(j)) {
                entries.add(createEntry(yValues, j, xAccessor, yAccessor));
            }
        }
        return entries;
    }
    ...
    abstract U createEntry(ReadableArray values, int index, String xAccessor, String yAccessor);
