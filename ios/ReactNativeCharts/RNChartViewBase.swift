...
open class RNChartViewBase: UIView, ChartViewDelegate {
  ...
  override open func didSetProps(_ changedProps: [String]!) {
    ...
    func setDragOffset(_ enabled: Bool) {
      if (enabled) {
        chart.viewPortHandler.setDragOffsetX(chart.viewPortHandler.contentWidth / 2)
      } else {
        chart.viewPortHandler.setDragOffsetX(0)
      }
    }
